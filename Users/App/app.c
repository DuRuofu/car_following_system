...
void Problem6(void)  //s型
{
	...
	{
		K210();
		Target_Speed_A = 35; 				//移动直到光源在中心
		Target_Speed_B = 35; 
		float x_err = (x - x_location)/3;
		if(x_err > 0)  //光向右，车向右，给左轮大速度
		{
			Target_Speed_A = 35 + x_err;
		}
		if(x_err < 0)  //光向左，车向左，给右轮大速度
		{
			Target_Speed_B = 35 - x_err;
		}
